...
$$d_T = frac{n_{commute} \times f_{commute} \times \bar{d}}{p_{commute} $$

```{r, echo=FALSE, eval=FALSE}
320 * 4.2 * 3
```

where $dT$ is the total distance cycled per year in each zone,
$n_{commute}$ is the number of bicycle commuters, $f$ is the frequency
...
results in 4,032 km of total distance cycled,  
...
2.994 billion vehicle kilometres (bkm) across England (there are 742,675 cycle commuters nationwide):
it is reassuring to note that the DfT's official estimate of total distance cycled
nationally is of the same order - 3.1 bkm (Keep, 2013) - although the official
