...
killometres cycled, based on census commuting statistics,
roughly double the national average.
This riskiness varies spatially and temporally, broadly in line
with expectations from the literature. An unexpected result was that
...
2/3rd of all trips were within easy cycling distance (between 0 and 5 miles)
...
and in some cases this may be justified.
Whilst the balance between the risks
and reward from cycling are strongly positive
straightforward overall,
there are contexts where this is not the case. 
...
However, the societal benefits depended on *who* switch mode:
...
foot or by bicycle is more than 17 times higher the by car (Davies,
2014).
...
'slight' injuries, which are likely heavily to be under-reported)
([Keep, 2013](http://www.parliament.uk/briefing-papers/SN06224.pdf)).
For this reason we focus on serious and fata injuries
in the exposure-normalised
risk estimates for comparison with national figures. 'Slight' incidents
account for 80% of the records, however, so are used unless otherwise stated.
...
from its current value of £1.30 per person per year to £5 by 2026
with a target of 300%
...
*600%* increase nationwide, far more ambitioius than West Yorkshire's target.
...
per year nationwide.
- The diurnal and seasonal distribution of incidents was expected to
...
West Yorkshire, a ratio found in highly gendered modes of transport
...
were involved in Motorway incidents
(cycling on Motorways is not permitted) (Fig. 8).
...
```{r fig.cap="Time of incident for cyclist and non-cyclist casualties by age band", fig.width=4, fig.height=3, echo=FALSE}
library(png)
library(grid)
...
The policy relevance of the findings is hightened by recent investment
...
Why is disproportionately riskier for vulnerable groups than others?
...
relative risks of cycling, shedding new light on
the local characteristics associated with
...
are disaggregated by severity, this effect dropped. 
- Fast roads are highly risky for cyclists.
