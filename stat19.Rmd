---
title: 'The shifting demographic and geographic distribution of bicycle crashes: a
  case study of West Yorkshire'
output:
  pdf_document: 
    fig_caption: true
    fig_height: 3
...
Factors associated with cycle safety, including international differences in injury and mortality rates, protective equipment and bicycle training, have been subject to increasing academic interest. Environmental factors associated with cycle safety have also been scrutinised, but few studies have
...
area as it has a historically low cycling rate but very ambitions
...
killometres cycled, based on the number of bicycle commuters in the region.
This is roughly double the national average in both cases.
This riskiness estimate varied spatially, with areas in Bradford and Kirklees
found to be 'danger hotspots'. Overall we provide evidence to suggest
local-level geographical factors do play an important role in cycle safety
and provide suggestions of how to increase cycle safety in the area.

# Introduction

The recent Annual Report of the UK's Chief Medical Officer, Dame Sally Davies, emphasised the need to promote active travel from a health perspective:
...
(ibid.,
Tables NTS0301 to NTS305).
This is despite the fact that the fact that roughly
2/3rd of all trips were within cycling distance (between 0 and 5 miles)
...
This may be rational in some cases: whilst the balance between the risks
and reward from cycling may appear at first glance to be
straightforward overall,
there are contexts where this is not the case. Although all-cause
...
The authors point out that while contemporary debates have focussed on the
need to improve cycling for health reasons, little has been said about the
potential increase in health *risks* associated with a rising number of
cyclist casualties (Short and Caulfield, 2014).
Mirroring these findings, official documents suggest that only casualties classed as
fatal and to a lesser extent serious can be relied upon from STATS19 (as opposed to
'slight' injuries, which are likely heavily under-reported)
([Keep, 2013](http://www.parliament.uk/briefing-papers/SN06224.pdf)).
For this reason we omit 'slight' injuries from the estimates of exposure-normalised
risk in the subsequent analysis.
...
such as is the case for the cycle hire scheme in Barcelona
...
positive overall but to disappear for young women,
...
a population of 2.2 million (from the 2011 Census), occupying an
...
are home to two of the largest cycling clubs in the country - ref???)]
...
calls for Leeds (historically known as the 'Motorway city of the Seventies')
to become a 'cycling city'. A spend of £30 million on cycling
between 2014 and 2017 is planned, including
£18 million from Department for Transport (DfT) through the
...
The West Yorkshire Integrated Transport Authority (WYITA) has 
committed to raise expenditure on cycling
from its value of £1.30 per person per year to £5 by 2026
(West Yorkshire Public Transport Executive, 2014). Associated
with this money are specific targets: a 300%
increase in the rate of cycling cycling across West Yorkshire
...
with an associated target for
cycling to account for 10% of all trips nationwide by 2025 (Goodwin, 2013).
Given the current modal share of 1.7%, this accounts for roughly a
*600%* increase nationwide. This is double West Yorkshire's equivalent target.
(Is it really West Yorkshire??? or perhaps the area directly influenced by
...
There is local ambition to maximise the long-term benefit from what will
be the largest free sporting event in the region's history.
It has been suggested that the investment will capitalise
...
Based on local surveys, cycling overall is reported to have grown
by only 0.9% (from what to what???) in Leeds and remained stable in Bradford at 0.8%
(WYITA, 2013). The same document reports that
4.4% of respondents took up to cycling in 2012.


```{r fig.cap="Overview of the study area", fig.width=6, fig.height=8, echo=FALSE}
library(png)
library(grid)
...
- Young adults (below 25) and the elderly (over 65) will be more likely to be involved in crashes
([Martinez-Ruiz et al., 2014](http://www.sciencedirect.com/science/article/pii/S0001457513004089)) than the population at large.
...
surrounding bridges without cycling facilities,
...
in each geographic zone (Equation 1):
...
In the context of the 1.3% rate of cycling to work in the area,
less than half the national average of 3.2% (Goodman, 2013),
this is unexpectedly high.

There has been a continuous increase in the proportion of
incidents involving cyclists in West Yorkshire (Fig. 2).
...
national data in accidents and may be accounted for by
increased uptake of cycling compared
with stagnating or declining distance travelled by other modes.
However, the increase cannot be completely accounted for by
increases in the proportion of distance travelled by bicycle nationwide
which increased by 57% (from 0.50% to 0.79%) over the same period.
...
```{r fig.cap="Map illustrating the mismatches between 2011 Local Authorities and 2001 Unitary Authorities and Districts", fig.width=6, fig.height=8, echo=FALSE}
library(png)
library(grid)
...
```{r fig.cap="Cyclist and non cyclist casualties from road traffic incidents, 2005 - 2012 in West Yorkshire, by age and severity. Note the variably sized age bins, which down-play the relative risks of road crashes for young people", fig.width=6, fig.height=8, echo=FALSE}
library(png)
library(grid)
...
(Fig. 4). This cannot be explained by age structure alone
...
unsafe place to be a young cyclist or walker, raising the question: why?
More data on the age distribution of
exposure regionally is needed to properly explore this.

```{r fig.cap="Anomaly in the age distribution of cyclist casualties in West Yorkshire, compared with the UK", fig.width=6, fig.height=8, echo=FALSE}
library(png)
library(grid)
...
The 2002 - 2008 National Travel Survey provides regional breakdowns to the
level of Government Office Regions. 
Using the proportion of trips made by bicycle for each age group as
a proxy of exposure, we calculated odds ratios for the riskiness of
bicycle trips compared with all trips by age group in West Yorkshire
(Table 1) and nationally (Table 2).
...
disproportionately greater for young people in in West Yorkshire
compared with the nation as a whole.
More localised age-specific exposure
data are needed to verify this result,
but, in combination with the corroborating findings presented in Fig. 4,
they present a worrying pattern of risk for young people in the country.
...
West Yorksire, a ratio found in highly gendered modes of transport
such as the motorbike (Table 3). This finding underlines
the gendered nature of cycling. These figures alone
...
and the ratio may be higher for distance cycled,
especially when sport cycling, a male dominated activity, is included.^[There
...
proportion of cycle commuters where were males in West Yorkshire (86%).]
...
follows the road network, as do incidents with no cyclist casualties (Fig. 5).
...
(Wickham, 2011). This is presented in Fig. 6, which shows the extent of
...
```{r fig.cap="Overview of the spatial distribution of accidents with cyclists and no cyclists involved", fig.width=6, fig.height=8, echo=FALSE}
library(png)
library(grid)
...
```{r fig.cap="Kernel Density Estimates for incidents involving cyclists (left) and no cyclists (right)", fig.width=6, fig.height=8, echo=FALSE}
library(png)
library(grid)
...
Table 4: Summary statistics on cycle commuting, serious and fatal incidents involving cyclists
...
Note that although Leeds has the highest number overall cyclist casualties
(and second highest number of casualties per capita), the results
suggest this is due to hightened exposue: central Leeds seems a comparatively
safe place to cycle in the context of West Yorkshire. All Local Authorities
are more risky
...
at lower geographic levels (Fig. 7). Because of the
...
```{r fig.cap="Risk of cycling per billion kilometres across West Yorkshire. The circles represent 5 km bands from the city centres which are deemed 'cyclable' by the majority of the population", fig.width=6, fig.height=8, echo=FALSE}
library(png)
library(grid)
...
estimated *risk* presented in Fig. 7 is very different from the distribution
of crashes overall (Fig. 6). This emphasises the importance of normalising
...
were involved Motorway incidents
(cycling on Motorways is not permitted) illegal (Fig. 8).
...
```{r fig.cap="Number of road incidents in West Yorkshire with cyclists (top) and no cyclists (bottom) involved by road class", fig.width=6, fig.height=8, echo=FALSE}
library(png)
library(grid)
...
## Timing of incidents injuring cyclists
...
rush hour almost 50% higher for cyclists than non cyclists (Fig. 9).

![timing of cyclist accidents](figures/cyclist-timings.png)

### Who is at risk when?

Cyclists seem disproportionately at risk during the rush hour (i.e. on the way to work). Subsequently, we explored whether the bimodal distribution displayed in Fig. 9 may be patterned by age. This
...
road traffic incidents (Fig. 10).
This pattern continues until retirement age (not shown).

```{r fig.cap="Time of incident for cyclist and non-cyclist casualties by age band", fig.width=6, fig.height=8, echo=FALSE}
library(png)
library(grid)
...
This paper explores the social and spatial characteristics of
cyclist casualties in a region with a historically low cycling rate.
The policy relevance of the findings are hightened by recent investment
in bicycle infrastructure and promotion in the area.
The danger is clear: people are encouraged to
cycle in dangerous environments, the rates of
death and serious injury for cyclists could increase.
Under a worst-case scenario, which has been alluded to for female cyclists in London
(Woodcock et al., 2014), the health costs of cycling could outweigh
the benefits for vulnerable groups. This could have a further
damaging knock-on impact by decreasing uptake of cycling due to media
reporting of collisions and reinforcement of a major deterant to cycling:
the perception that it is risky.

The analysis indicates that cycling in West Yorkshire is roughly two
times riskier than
cycling nationwide. This, in combination with the regions great
cycling ambitions and planned investments make
the danger more pronounced in the area. Children cyclists seem to be
disprortionally at risk, both in terms of the proportion of
incidents involving cyclings and estimates of the proportion of
trips resulting in incidents, compared with the equivalent figures nationwide.
This should be cause for concern given that the behaviour of young people
today will have impacts long into the future and the disproportionate impact
on disability adjusted life years (DALYs) of young casualties (Woodcock et al., 2014).
Assuming the finding is robust, this also raises an unexplored research question:
Why disproportionately riskier for vulnerable groups than others?

In addition to describing the pattern of bicycle crashes in West Yorkshire,
the paper has also explored a number of hypotheses related to the
relative risks of cycling.
...
whilst others received limited support:

- Our findings support the hypothesis that young cyclists are at greater risk from
road traffic than adults. West Yorkshire seems to be especially risky for young
people, although this finding needs to be corroborated by better exposure data before
strong conclusions. 
- We found evidence to support the hypothesis that cyclists were disproportionately
at risk at junctions and roundabouts than other road users. However, when incidents
are disaggregated by severity, this effect dropped. Fast roads are highly risky for
cyclists.
- We found some evidence to support the idea that cycling is safer where more people
cycle. However, our local exposure estimates are based solely on cycle commuting, so
this finding requires further research to be corroborated.
- The proportion of incidents involving cyclists has
increased over time, in line with prior expectations. However, the
increase happened faster than growth in the proportion of all distance
travelled by bicycle nationwide, implying that cycling in West Yorkshire has
become more dangerous over time. Again, better localised (and time series)
exposure estimates are needed to verify this result.
- The diurnal distribution of bicycle incidents was indeed found to be different
from the national average, with a more 'peaky' and bimodal distribution than for
other non-cyclist incidents. This implies that cyclists may be disproportionately
affected by the heavy traffic conditions of the 'rush hour'.
...
On the other hand, there is evidence that measures targetting
saftety specifically improve *perceptions* of safety
...
These changes will not only improve the health balance of cycling:
they can, through encouraging a wider shift to active travel
provide benefits throughout society.
...
Woodcock, J., Tainio, M., Cheshire, J., O\textquoterightBrien, O., & Goodman, A. (2014). Health effects of the London bicycle sharing system: health impact modelling study. BMJ, 348. doi:10.1136/bmj.g425
