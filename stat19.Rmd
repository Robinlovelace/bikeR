...
lost from traffic incidents in the Netherlands (de Hartog et al., 2010).
...
This is an issue as it means the nature of the data is selective, insofar as incidents are more likely to be recorded the more severe they are (Langley et al., 2003). Furthermore, as there is no legal obligation to report incidents to the police, many occur without the involved parties reporting an incident. Moreover, if there is no injury, the incident is not deemed 'reportable' (DfT, 2013). 
...
as the severity the incident increases: 30% of serious cyclist injuries
...
The temporal distribution of cyclist casualties is highly correlated with that of road traffic incidents overall. However, crashes involving cyclists have a much ‘peakier' distribution than that all other road traffic incidents, with the incident density during the afternoon rush hour almost 50% higher for cyclists than non-cyclists (Fig. 11).
...
```{r, eval=FALSE}
system("latexdiff stat19.tex stat19-submitted.tex > dif.tex")
```
